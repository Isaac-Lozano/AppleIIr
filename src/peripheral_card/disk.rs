...
impl Drive
{
    ...
    pub fn add_disk(&mut self, disk: &mut Read)
    {
        ...
        for (track_num, track) in data.iter_mut().enumerate()
        {
            for (sector_num, sector) in track.iter_mut().enumerate()
            {
                ...
                /* address header */
                sector[idx] = 0xD5; idx += 1;
                sector[idx] = 0xAA; idx += 1;
                sector[idx] = 0x96; idx += 1;

                /* disk volume = 254 */
                sector[idx] = 0xFF; idx += 1;
                sector[idx] = 0xFE; idx += 1;

                sector[idx] = Drive::nib_odd(track_num as u8); idx += 1;
                sector[idx] = Drive::nib_even(track_num as u8); idx += 1;

                sector[idx] = Drive::nib_odd(phys_sector); idx += 1;
                sector[idx] = Drive::nib_even(phys_sector); idx += 1;

                let checksum = 254 ^ track_num ^ phys_sector as usize;
                sector[idx] = Drive::nib_odd(checksum as u8); idx += 1;
                sector[idx] = Drive::nib_even(checksum as u8); idx += 1;

                /* address trailer */
                sector[idx] = 0xDE; idx += 1;
                sector[idx] = 0xAA; idx += 1;
                sector[idx] = 0xEB; idx += 1;
                ...
                for off in 0..0x56
                {
                    let i = (buf[off + 0x56] & 3)
                          | (buf[off + 0x56 + 0x56] & 3) << 2
    ...
    fn step_motor(&mut self, magnet: u16, enable: bool)
    {
        ...
        if self.magnets & (1 << ((self.phase + 1) % 4)) != 0
        && self.phase < 140
        {
            self.phase += 1;
        }
        if self.magnets & (1 << ((self.phase + 3) % 4)) != 0
        && self.phase > 0
        {
            self.phase -= 1;
        }
...
impl PeripheralCard for DiskII
{
    ...
    fn read_switch_without_mm(&mut self, switch: u16) -> u8
    {
        match switch
        {
            ...
            0x0C =>
            {
                ...
                {
                    Mode::Read =>
                        self.current_drive().read_without_mm(),
                    Mode::Write => 0,
                }
            },
            0x0D =>
    ...
    fn read_expansion_rom(&mut self, _addr: u16) -> u8 {0}
}
